...
function load(currentId) {
	var currentChild = getChildrenById(data, currentId); //要渲染的子集
	var currentPar = getParentsById(data, currentId); // 当前目录 以及 所有父级
	//生成文件夹
...
(function () {
	...
	function pick(e, pickType) {
		...
		if (!parent) return;
		//自定义属性data-index获取id
		var currentId = parent.dataset.index;
	...
	fileBox.addEventListener('dblclick', function (e) {
		...
		if (parent && target.nodeName !== 'INPUT' && target.nodeName !== 'SPAN') {
			var currentId = parent.dataset.index; //通过图片或者是其他元素找到父级fileitem
			var current = getDataById(data, currentId);

			//进入文件夹，取消被选中样式，加载导航栏和文件夹
...
listBox.addEventListener('click', function (e) {
	...
	if (target.className !== 'list') {
		...
		var currentId = parent.dataset.index;
		//数据的checked属性初始化;
		var currentChild = getChildrenById(data, currentId); //找到它父级 循环children 数据里面的checked = false
		for (var i = 0; i < currentChild.length; i++) {
			currentChild[i].checked = false;
		};
...
navBox.addEventListener('click', function (e) {
	...
	if (target.nodeName === 'A') {
		...
		var currentId = target.className === 'back' ? getDataById(data, nowId).pId : target.dataset.index; //判断点的是返回上一级还是其他
		nowId = currentId;
		load(nowId);
	};
}, false);
...
(function () {
	fileWrap.onmousedown = function (e) {
		...
		var startX = e.pageX - main.offsetLeft,
			startY = e.pageY - fileWrap.offsetTop; //鼠标点击时的坐标
		...
		fileWrap.onmousemove = function (e) {
			...
			var currentX = e.pageX - main.offsetLeft,
				currentY = e.pageY - fileWrap.offsetTop;
